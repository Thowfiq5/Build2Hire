...
// Global API Route Helper
window.getApiUrl = function(endpoint) {
  const cleanEndpoint = endpoint.startsWith('/') ? endpoint.slice(1) : endpoint;
  
  // 1. If running on standalone PHP dev server (port 8000)
  if (window.location.port === '8000') {
    return '/' + cleanEndpoint;
  }
  
  // 2. If running under XAMPP Apache (e.g. http://localhost/Build2Hire/...)
  const pathParts = window.location.pathname.split('/');
  if (pathParts[1] && pathParts[1].toLowerCase() === 'build2hire') {
    return '/Build2Hire/' + cleanEndpoint;
  }
  
  // 3. Fallback for VS Code Live Server (port 5500/5501/etc) or file:// protocol
  // Live Server cannot execute PHP files, so direct API calls to PHP server on port 8000
...
const SAMPLE_CLIENT_JOBS = [
  {
    id: "job_seed_101",
    title: "Full Stack Developer",
    description: "Build scalable web applications with React frontend and Node.js RESTful APIs.",
    budget: 3000,
    duration: "2 Months",
    ...
    postedBy: 1004,
    posted_by: 1004,
    postedByName: "Sarah Connor (Hiring Manager)",
  ...
  },
  {
    ...
    description: "Design clean wireframes and high-fidelity Figma mobile application prototypes.",
    budget: 1500,
    duration: "1 Month",
    ...
    status: "open",
    type: "Freelance",
    createdAt: 1700000000000 + 86400000
  },
  {
    id: "job_seed_103",
    title: "Backend API Developer",
    description: "Develop secure Node.js APIs integrated with PostgreSQL and JWT authentication.",
    budget: 2500,
    duration: "6 Weeks",
    ...
    status: "open",
    type: "Freelance",
    createdAt: 1700000000000 + 172800000
  },
  {
    id: "job_seed_104",
    title: "WordPress Website Revamp",
    description: "Custom PHP template creation and responsive CSS styling for corporate website.",
    budget: 800,
    duration: "3 Weeks",
    ...
    status: "open",
    type: "Freelance",
    createdAt: 1700000000000 + 259200000
  },
  {
    id: "job_seed_105",
    title: "React Native Mobile App",
    description: "Cross-platform mobile app development with React Native and Firebase integration.",
    budget: 4000,
    duration: "3 Months",
    ...
    status: "open",
    type: "Freelance",
    createdAt: 1700000000000 + 345600000
  }
];
...
const DEFAULT_APPLICATIONS = [
  ...
  },
  {
    ...
    candidateName: "Test 1",
    status: "Applied",
    appliedAt: "2026-09-12T14:30:00.000Z"
  }
];
...
function dbGet() {
  ...
  if (!parsed.jobs || parsed.jobs.length === 0) {
    ...
    SAMPLE_CLIENT_JOBS.forEach(sj => {
      const idx = parsed.jobs.findIndex(j => String(j.id) === String(sj.id));
      if (idx === -1) {
        parsed.jobs.unshift(sj);
        updated = true;
      } else {
        if (parsed.jobs[idx].postedBy !== sj.postedBy || parsed.jobs[idx].category !== sj.category) {
          parsed.jobs[idx] = { ...parsed.jobs[idx], ...sj };
          updated = true;
        }
      }
    });
    
    // Deduplicate parsed.jobs by ID
    ...
    if (cleanJobs.length !== parsed.jobs.length) {
      parsed.jobs = cleanJobs;
      updated = true;
    }
  }
...
function getUser() {
  try {
    let session = sessionStorage.getItem('user') || localStorage.getItem('user');
    if (!session) {
      return null;
    }
    ...
    if (latestUser) {
      ...
      if ((!latestUser.skills || latestUser.skills.length === 0) && latestUser.role === 'candidate') {
      ...
      if ((!latestUser.enrolled_courses || latestUser.enrolled_courses.length === 0) && latestUser.role === 'candidate') {
...
function withAuth(allowedRoles = ['candidate', 'recruiter', 'client', 'admin']) {
  const user = getUser();
  if (!user) {
  ...
  const targetCandidateId = urlParams.get('candidateId') || urlParams.get('id') || urlParams.get('view');
  const path = window.location.pathname.split('/').pop() || "index.html";
  const isPortfolioView = !!(targetCandidateId && (path === 'portfolio.html' || path === 'portfolio'));

  // Always allow admin access to any page
  // Also allow clients/recruiters to view portfolio.html when targetCandidateId is passed
  if (user.role !== 'admin' && !allowedRoles.includes(user.role) && !isPortfolioView) {
    ...
    else if (user.role === 'candidate') window.location.href = "portfolio.html";
  ...
  // Resolve target candidate override if viewing another candidate's profile/portfolio
  if (targetCandidateId) {
    const foundCand = db.users.find(u => String(u.id) === String(targetCandidateId));
    if (foundCand) {
      candidate = foundCand;
    }
  }
...
function renderHeader() {
  ...
  if (currentPath === 'login.html' || currentPath === 'login' || currentPath === 'register.html' || currentPath === 'register' || currentPath === 'reset.html' || currentPath === 'reset') {
    ...
    nav.innerHTML = `
      <div style="display:flex;align-items:center;gap:1rem;">
        <a href="login.html" onclick="window.handleLogoClick(event, 'login.html')" ondblclick="window.openAdminLoginModal()" title="Build2Hire" style="font-family: var(--font-display); font-size: 1.6rem; font-weight: 800; color: var(--text-primary); text-decoration: none; letter-spacing: -0.02em;">
      ...
      <div style="display:flex;align-items:center;gap:1rem;">
        <a href="login.html" class="nav-link" style="font-size:0.9rem; font-weight: 600;">🏠 Home</a>
        ${isLogin 
  ...
  const dashboardPage = user
    ? (user.role === 'admin' ? 'admin-dashboard.html' : (user.role === 'recruiter' ? 'recruiter-dashboard.html' : (user.role === 'client' ? 'client-dashboard.html' : 'portfolio.html')))
    : 'login.html';

  const userControls = user
    ? `<a href="${user.role === 'candidate' ? 'profile.html' : dashboardPage}" class="btn btn-secondary" style="padding:0.5rem 1rem;font-size:0.85rem;">👤 ${user.fullName || 'Dashboard'}</a>
  ...
  nav.innerHTML = `
    ...
    <nav class="desktop-nav" style="display:flex;align-items:center;gap:1.5rem;">
      <ul class="nav-links">
        ...
        ${user && (user.role === 'recruiter' || user.role === 'admin') ? '<li><a href="leaderboard.html" class="nav-link">🏆 Leaderboard</a></li>' : ''}

        ${user && user.role === 'candidate' ? '<li><a href="about.html" class="nav-link">About Us</a></li>' : ''}
        ...
        ${user && user.role === 'candidate' ? '<li><a href="recommendations.html" class="nav-link">📚 Learning Hub</a></li>' : ''}
      ...
      <div style="display:flex;align-items:center;gap:1rem;">
        ${user && user.role === 'candidate' ? `
          <div id="cand-notif-wrapper" style="position: relative; display: inline-block;">
            ...
            <span id="cand-notif-badge" style="position: absolute; top: -3px; right: -3px; background: #ef4444; color: white; font-size: 0.65rem; font-weight: 800; padding: 2px 5px; border-radius: 10px; min-width: 16px; text-align: center; display: none; z-index: 10; pointer-events: none;">0</span>
        ...
        ${user && user.role === 'recruiter' ? `
          <div id="rec-notif-wrapper" style="position: relative; display: inline-block;">
            ...
            <span id="rec-notif-badge" style="position: absolute; top: -3px; right: -3px; background: #a855f7; color: white; font-size: 0.65rem; font-weight: 800; padding: 2px 5px; border-radius: 10px; min-width: 16px; text-align: center; display: none; z-index: 10; pointer-events: none;">0</span>
        ...
        ` : ''}
          ...
          ${user && user.role === 'client' ? `
            <div id="client-notif-wrapper" style="position: relative; display: inline-block;">
              <button id="client-notif-bell-btn" onclick="window.toggleClientNotificationDropdown(event)" class="theme-toggle" style="position: relative; cursor: pointer; border: 1px solid var(--border-color); background: var(--bg-tertiary); color: var(--text-primary); font-size: 1.15rem; width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center; transition: all 0.2s;" title="View Client Notifications">
                🔔
              </button>
              <span id="client-notif-badge" style="position: absolute; top: -3px; right: -3px; background: var(--primary); color: white; font-size: 0.65rem; font-weight: 800; padding: 2px 5px; border-radius: 10px; min-width: 16px; text-align: center; display: none; z-index: 10; pointer-events: none;">0</span>
  
              <!-- CLIENT NOTIFICATION DROPDOWN MENU -->
              <div id="client-notif-dropdown" style="display: none; position: absolute; right: 0; top: 48px; width: 340px; max-height: 480px; background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.35); z-index: 999999; overflow: hidden; flex-direction: column;">
                <div style="padding: 0.85rem 1rem; border-bottom: 1px solid var(--border-color); background: var(--bg-tertiary);">
                  <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div>
                      <strong style="font-size: 0.92rem; color: var(--text-primary); font-family: var(--font-display);">🔔 Notifications</strong>
                      <span id="client-notif-unread-text" style="font-size: 0.72rem; color: var(--primary); display: block; font-weight: 600;">0 unread</span>
                    </div>
                    <div style="display: flex; align-items: center; gap: 0.5rem;">
                      <button onclick="window.markAllClientNotificationsRead()" class="btn-xs btn-secondary" style="font-size: 0.7rem; padding: 0.25rem 0.5rem;">Mark All Read</button>
                      <button onclick="window.toggleClientNotificationDropdown(event)" style="background: none; border: none; cursor: pointer; color: var(--text-muted); font-size: 1.1rem; padding: 0.1rem 0.35rem; border-radius: 4px; display: flex; align-items: center; justify-content: center;" title="Close Menu">✖</button>
                    </div>
                  </div>
                </div>
  
                <!-- NOTIFICATION ITEMS CONTAINER -->
                <div id="client-notif-list" style="overflow-y: auto; max-height: 340px; display: flex; flex-direction: column;">
                  <!-- Populated dynamically -->
                </div>
  
                <div style="padding: 0.6rem 1rem; border-top: 1px solid var(--border-color); text-align: center; background: var(--bg-tertiary);">
                  <button onclick="window.clearAllClientNotifications()" style="font-size: 0.75rem; color: var(--danger); background: none; border: none; cursor: pointer; font-weight: 600;">🗑️ Clear All Notifications</button>
                </div>
              </div>
            </div>
          ` : ''}
        ...
        ${user && user.role === 'admin' ? `
          <div id="admin-notif-wrapper" style="position: relative; display: inline-block;">
            ...
            <span id="admin-notif-badge" style="position: absolute; top: -3px; right: -3px; background: var(--primary); color: white; font-size: 0.65rem; font-weight: 800; padding: 2px 5px; border-radius: 10px; min-width: 16px; text-align: center; display: none; z-index: 10; pointer-events: none;">0</span>
    ...
    <div class="mobile-menu-drawer" id="mobile-menu-drawer">
      ...
      <ul style="list-style:none; padding: 1.5rem; display:flex; flex-direction:column; gap:1.5rem; font-size:1.1rem;">
        ...
        ${user && (user.role === 'recruiter' || user.role === 'admin') ? '<li><a href="leaderboard.html" onclick="toggleMobileMenu()">🏆 Leaderboard</a></li>' : ''}
        ${user && user.role === 'candidate' ? '<li><a href="about.html" onclick="toggleMobileMenu()">About Us</a></li>' : ''}
        ...
        ${user && user.role === 'candidate' ? '<li><a href="recommendations.html" onclick="toggleMobileMenu()">📚 Learning Hub</a></li>' : ''}
        ${user && user.role === 'admin' ? '<li><a href="admin-dashboard.html" onclick="toggleMobileMenu()">⚙️ Admin Dashboard</a></li>' : ''}
        ${user ? `<li><a href="${user.role === 'candidate' ? 'profile.html' : dashboardPage}" onclick="toggleMobileMenu()">👤 Dashboard (${user.role})</a></li>` : ''}
  ...
  if (user && user.role === 'candidate') {
    ...
    setTimeout(window.renderClientNotificationCenter, 50);
...
// ==========================================
// 🔔 CLIENT NOTIFICATION CENTER CONTROLLERS
// ==========================================

window.getClientNotifications = function() {
  const user = getUser();
  if (!user) return [];
  const db = dbGet();
  
  if (!db.client_notifications || db.client_notifications.length === 0) {
    let defaultMsg = "Your registration is pending admin approval. This usually takes up to 24 hours.";
    let defaultTitle = "⏳ Registration Under Review";
    let defaultCategory = "status";
    
    if (user.status === 'approved') {
       defaultTitle = "✅ Registration Approved";
       defaultMsg = "Your client account has been approved. You can now access your dashboard.";
    } else if (user.status === 'rejected') {
       defaultTitle = "❌ Registration Rejected";
       defaultMsg = "Your client registration was declined.";
    }
  ...
  const userNotifs = db.client_notifications.filter(n => String(n.userId) === String(user.id) || !n.userId);
  return userNotifs.sort((a,b) => b.timestamp - a.timestamp);
};

window.renderClientNotificationCenter = function() {
  const notifs = window.getClientNotifications();
  const unreadCount = notifs.filter(n => !n.read).length;

  const badge = document.getElementById('client-notif-badge');
  const unreadText = document.getElementById('client-notif-unread-text');
  const listElem = document.getElementById('client-notif-list');
    ...
    if (notifs.length === 0) {
      ...
      listElem.innerHTML = notifs.map(n => {
        ...
        return `
          <div style="padding: 0.85rem 1rem; border-bottom: 1px solid var(--border-color); background: ${n.read ? 'transparent' : 'rgba(168,85,247,0.08)'}; display: flex; gap: 0.75rem; align-items: flex-start; transition: background 0.15s; position: relative;">
            <div style="font-size: 1.1rem; flex-shrink: 0; margin-top: 2px;">
              ${n.category === 'status' ? '⏳' : (n.category === 'success' ? '✅' : '🔔')}
            </div>
            <div style="flex: 1;">
              <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.2rem;">
                <strong style="font-size: 0.82rem; color: var(--text-primary);">${n.title}</strong>
                <button onclick="window.dismissClientNotification('${n.id}', event)" style="background: none; border: none; cursor: pointer; color: var(--text-muted); font-size: 0.8rem; padding: 0 4px;" title="Dismiss">✖</button>
              </div>
              <p style="font-size: 0.78rem; color: var(--text-secondary); margin: 0 0 0.3rem 0; line-height: 1.35;">${n.message}</p>
              <span style="font-size: 0.7rem; color: var(--text-muted);">${timeAgo} ${!n.read ? '• <strong style="color:#a855f7;">Unread</strong>' : ''}</span>
            </div>
...
window.toggleClientNotificationDropdown = function(e) {
  if (e) e.stopPropagation();
  const dropdown = document.getElementById('client-notif-dropdown');
  if (!dropdown) return;
  const isHidden = dropdown.style.display === 'none' || !dropdown.style.display;
  dropdown.style.display = isHidden ? 'flex' : 'none';
  if (isHidden) window.renderClientNotificationCenter();
};

window.markAllClientNotificationsRead = function() {
  const db = dbGet();
  const user = getUser();
  if (!user || !db.client_notifications) return;

  db.client_notifications.forEach(n => {
    if (String(n.userId) === String(user.id) || !n.userId) {
      n.read = true;
    }
  });
  ...
  dbSave(db);
  window.renderClientNotificationCenter();
};

window.clearAllClientNotifications = function() {
  const db = dbGet();
  const user = getUser();
  if (!user || !db.client_notifications) return;

  db.client_notifications = db.client_notifications.filter(n => String(n.userId) !== String(user.id));
  dbSave(db);
  window.renderClientNotificationCenter();
};

window.dismissClientNotification = function(notifId, e) {
  if (e) e.stopPropagation();
  const db = dbGet();
  if (!db.client_notifications) return;

  db.client_notifications = db.client_notifications.filter(n => String(n.id) !== String(notifId));
  dbSave(db);
  window.renderClientNotificationCenter();
};

window.addClientNotification = function(title, message, category = 'system') {
  const user = getUser();
  if (!user) return;
  const db = dbGet();
  db.client_notifications = db.client_notifications || [];
  ...
  db.client_notifications.unshift(newNotif);
  dbSave(db);
...
document.addEventListener('click', function(e) {
  ...
  const clientWrapper = document.getElementById('client-notif-wrapper');
  const clientDropdown = document.getElementById('client-notif-dropdown');
  if (clientWrapper && clientDropdown && !clientWrapper.contains(e.target)) {
...
function renderSidebar(activePage) {
  ...
  if (user.role === 'candidate' && (activePage === 'portfolio.html' || activePage === 'portfolio')) {
      ...
      <div style="display: flex; flex-direction: column; gap: 1.25rem;">
        <div>
          ...
          <ul class="sidebar-menu" style="display: flex; flex-direction: column; gap: 0.15rem;">
            ...
            <li>
              <a href="admin-dashboard.html?tab=clients" onclick="if(window.switchTab){event.preventDefault();switchTab('clients');}" class="sidebar-link ${isDashboard && activeTab === 'clients' ? 'active' : ''}">
  ...
  if (user.role === 'client') {
    const db = dbGet();
    const freshUser = (db.users && db.users.find(u => u.email && user.email && u.email.toLowerCase() === user.email.toLowerCase())) || user;
    const isUnderReview = freshUser.status === 'pending' || freshUser.status === 'rejected' || freshUser.status !== 'approved';
    const lockBadge = isUnderReview ? `<span style="opacity: 0.75; font-size: 0.75rem; margin-left: auto;">🔒</span>` : '';
    const linkStyle = isUnderReview ? `opacity: 0.8;` : '';
    ...
    sidebar.innerHTML = `
      <div style="display: flex; flex-direction: column; gap: 2rem;">
        <div>
          ...
          <ul class="sidebar-menu">
            <li>
              <a href="client-dashboard.html" class="sidebar-link ${activePage === 'client-dashboard.html' ? 'active' : ''}">
                💼 <span>Client Dashboard</span>
              </a>
            </li>
            <li>
              <a href="chat.html" class="sidebar-link ${activePage === 'chat.html' ? 'active' : ''}" style="${linkStyle}">
                💬 <span>Inbox & Meetings</span> ${lockBadge}
              </a>
            </li>
            <li>
              <a href="agreement-builder.html" class="sidebar-link ${activePage === 'agreement-builder.html' ? 'active' : ''}" style="${linkStyle}">
                🤝 <span>Agreement Builder</span> ${lockBadge}
              </a>
            </li>
          </ul>
        </div>
...
function sendMessage(convId, senderId, text) {
  ...
  if (String(senderId) === String(conv.clientId)) {
    db.candidate_notifications = db.candidate_notifications || [];
    db.candidate_notifications.unshift({
      id: `cnotif-${Date.now()}`,
      userId: conv.candidateId,
      title: `New Message from ${conv.clientName}`,
    ...
    db.recruiter_notifications = db.recruiter_notifications || [];
    db.recruiter_notifications.unshift({
      id: `rnotif-${Date.now()}`,
  ...
  if (typeof window.renderCandidateNotificationCenter === 'function') window.renderCandidateNotificationCenter();
  if (typeof window.renderRecruiterNotificationCenter === 'function') window.renderRecruiterNotificationCenter();
...
function adminApproveCompany(compRef) {
  ...
  if (db.users && comp.candidate_id) {
    const user = db.users.find(u => String(u.id) === String(comp.candidate_id));
    if (user) {
      user.companyRegistrationStatus = 'approved';
      db.candidate_notifications = db.candidate_notifications || [];
      db.candidate_notifications.unshift({
        ...
        message: "Your company registration has been approved! You can now access Freelance Gigs.",
        category: "success",
        read: false,
        timestamp: Date.now()
      });
...
function adminRejectCompany(compRef, reason) {
  ...
  if (db.users && comp.candidate_id) {
    ...
    if (user) {
      user.companyRegistrationStatus = 'rejected';
      user.companyRejectionReason = comp.rejectionReason;
      db.candidate_notifications = db.candidate_notifications || [];
      db.candidate_notifications.unshift({
        id: `cnotif-${Date.now()}`,
        userId: user.id,
        title: "Company Registration Rejected",
        message: `Your company registration was rejected. Reason: ${comp.rejectionReason}`,
        category: "error",
        read: false,
        timestamp: Date.now()
      });
...
window.navigateSeamlessly = function(targetUrl, isPopState = false) {
  ...
  fetch(targetUrl)
    ...
    .then(htmlText => {
      ...
      // Update sidebar visibility/state based on new page
      if (typeof window.renderSidebar === 'function') {
        window.renderSidebar(targetPath);
      }
      ...
      scripts.forEach(s => {
        ...
        if (s.src) {
          ...
          // Convert top-level const/let declarations to var to prevent SyntaxError on SPA navigation
          let scriptBody = s.textContent.replace(/^\s*(const|let)\b/gm, 'var');
      ...
      setTimeout(() => {
        ...
        if (typeof window.initializeJobsUI === 'function' && targetPath === 'jobs.html') {
          window.initializeJobsUI();
        }
        if (typeof window.initializeRecommendationsUI === 'function' && targetPath === 'recommendations.html') {
          window.initializeRecommendationsUI();
        }
        if (typeof window.initializePortfolioUI === 'function' && targetPath === 'portfolio.html') {
          window.initializePortfolioUI();
        }
        if (typeof window.initializeCertificatesUI === 'function' && targetPath === 'certificates.html') {
          window.initializeCertificatesUI();
        }
        if (typeof window.initializeLeaderboardUI === 'function' && targetPath === 'leaderboard.html') {
          window.initializeLeaderboardUI();
        }
        if (typeof window.initializeFreelanceUI === 'function' && targetPath === 'freelance.html') {
          window.initializeFreelanceUI();
        }
        if (typeof window.initializeCandidateDashboardUI === 'function' && (targetPath === 'candidate-dashboard.html' || targetPath === 'profile.html')) {
          window.initializeCandidateDashboardUI();
        }
      }, 30);
