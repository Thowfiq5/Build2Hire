...
const withAuth = () => ({
  user: { id: 1001, role: 'candidate' },
  db: {
    users: [{ id: 1001, role: 'candidate', profileCompleted: true, preferredRole: 'Frontend Developer', xp_points: 0, level: 1 }],
    jobs: [
      { id: 1, title: 'Job 1', category: 'Frontend Developer', type: 'Full-time', skills: ['HTML'] }
    ],
    applications: []
  },
  candidate: { id: 1001, role: 'candidate', profileCompleted: true, preferredRole: 'Frontend Developer', xp_points: 0, level: 1 }
});
...
const setTimeout = (fn) => {};
    ...
    if (auth) {
      ...
      // Filter allJobs strictly to match the candidate's registered preferred role
      let allJobs = [];
      if (candidateRole) {
        allJobs = (db.jobs || []).filter(job => job.category.toLowerCase() === candidateRole.toLowerCase());
      }
      ...
      function renderJobs(list) {
        ...
        // Render AI Bonus Jobs (Non-matching roles)
        if (candidateRole) {
          const bonusGrid = document.getElementById('ai-bonus-jobs-grid');
          if (bonusGrid) {
            bonusGrid.innerHTML = "";
            let nonMatchedJobs = (db.jobs || []).filter(j => j.category.toLowerCase() !== candidateRole.toLowerCase());
            // Grab top 2 non-matching
            const bonusJobs = nonMatchedJobs.slice(0, 2);
            
            if (bonusJobs.length > 0) {
              bonusJobs.forEach(job => {
